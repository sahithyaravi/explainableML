...
def register_callbacks(app):
    ...
    @app.callback(Output('queries', 'children'),
                  ...
                  [State('datatable', 'selected_rows'),
                   State('select_dataset', 'value')])
    def get_queries_write_labels_radio(next_round,  selected_rows, dataset):
        output = ""
        ...
        if next_round is not None:
            ...
            if table_text:
                ...
                output = html.Div([checkall, display_table])
            else:
...
def fetch_queries(dataset, next_round, selected_rows):
    print(next_round, selected_rows)
    ...
    if next_round > 1:
        df["label"].loc[df["cluster_id"] == min_cluster] = 0
        labelled_df = df[df["cluster_id"] == min_cluster]
        if selected_rows:
            selected_rows = [r + labelled_df['index'][0] for r in selected_rows]
            labelled_df[labelled_df['index'].isin(selected_rows)] = 1
        print("after labelling", labelled_df)
